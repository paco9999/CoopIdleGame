...
describe("DungeonManager", function () {
    ...
    beforeEach(async function () {
        ...
        dungeonManager = await DungeonManager.deploy(addr1.address, await craftingManager.getAddress());
    });
    ...
    describe("Gestione degli errori", function () {
        ...
        it("Non dovrebbe permettere l'inizializzazione con numero di oggetti richiesti zero", async function () {
            const validItems = [1, 2, 3];
            for (const recipeId of validItems) {
                await craftingManager.setRecipeValidity(recipeId, true);
            }
            ...
            await expect(
                dungeonManager.initializeDungeon(1, validItems, [100, 200, 300, 400], 3600, 0)
            ).to.be.revertedWith("Il numero di oggetti richiesti deve essere maggiore di zero");
        });
